...
public final class EventPubDrivenPublisher extends Publisher {
    ...
    protected void initialLoad() throws APIException {
        ...
        TimerTask reportStats = new TimerTask() {
            ...
            public void run() {
                ...
                Logs.info("ADD " + avgAdd + "\t| REM " + avgRemove + "\t| PUB "
                        + avgPublish);
            }
            ...
            private String averageCost(AtomicLong cost, AtomicLong counter) {
                ...
                if (count == 0) {
                    ...
                    return "avg:" + (cost.getAndSet(0) / count) + "us,count:"
                            + count;
                }
            }
        };
    ...
    private void prepareMessage(TopicMessage message) throws MessageException {
        if (message.size() > fragmentThreshold) {
            message.setFragmentSize(fragmentSize);
            if (fragmentDelay > 0) {
                message.setFragmentedMessageLifecycle(new FragmentedMessageLifecycle(
                        fragmentDelay));
            }
        }
    }
