...
public class Orderlist {
	...
	public long matchOrder( Order addOrder, List<Trade> trades )
	{
		...
		//	log.debug( "Submitting order isBid:" + addOrder.getIsBid() + " price:" + addOrder.getPrice() + " isGTC:" + addOrder.getIsGTC() + " order:" + addOrder.getRemainingQty() + " orderId:" + addOrder.getOrderId() + " REF:" + addOrder );
		...
		// Match it
		Iterator< Map.Entry< Long, Order > > listIter = orderList.entrySet().iterator();
		...
		while ( listIter.hasNext() && addOrder.getRemainingQty() > 0 ) {
			...
			// Match it
			long matchedQty = Math.min( listEntry.getValue().getRemainingQty(), addOrder.getRemainingQty() );
			addOrder.match( listEntry.getValue(), listEntry.getValue().getPrice(), true,  matchedQty );
			listEntry.getValue().match( addOrder, listEntry.getValue().getPrice(), false, matchedQty );
