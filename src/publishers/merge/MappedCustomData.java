...
public class MappedCustomData implements CustomTopicDataHandler {
    ...
    public String asString() {
        return "Map of " + map.size() + " entries, " + newEntries.size()
                + " pending changes";
    }

    @Override
    public boolean endUpdate() throws APIException {
    ...
    @Override
    public void initialise(TopicMessage message) throws APIException {
        newEntries.clear();
        hasChanges = false;
        update(message);
    }
    ...
    @Override
    public void populateTopicLoad(TopicMessage topicLoad) throws APIException {
        (new MappedTopicMessage<String, String>(topicLoad)).addEntries(map
                .entrySet());
        ...
        for (Record rec : message.asRecords()) {
            if (rec.size() == 1) {
                ...
                if (currValue == null || !currValue.equals(rec.getField(1)))
                    newEntries.add(new RecordEntry(rec.getField(0), rec
                            .getField(1)));
            }
        }
