...
public class OrderSimulator {
	...
	static public void main( String[] args ) {
		...
		OrderMatcher om = new OrderMatcher( new OrderMatcherMonitor() {
			...
			@Override public void orderBookChanges(List<Trade> trades, List<OrderbookChange> bidChanges, List<OrderbookChange> askChanges, OrderMatcher om) {
				...
				//log.debug( "Best bid: " + (bids.isEmpty()?"NA":bids.lastEntry().getKey()) + " Best ask: " + (asks.isEmpty()?"NA":asks.firstEntry().getKey()) );
			}
			private void applyChanges( String side, TreeMap< Long, Long > map, List<OrderbookChange> changes ) {
				for ( OrderbookChange change : changes ) {
					//if ( log.isDebugEnabled() )
					//	log.debug( "Book change: " + side + " " + change.getPrice() + ": " + change.getNewQuantity() );
	...
	private void addToVWAP( final long price, final long qty ) {
		...
		//if ( log.isDebugEnabled() )
		//	log.debug( "VWAP has [" + price + "," + qty + "] as new trade, latest VWAP " + lastVWAP );
		...
		timer.schedule( new TimerTask() {
			@Override public void run() {
				...
				if ( vwapQty > 0 ) {
					lastVWAP = Math.round(vwapPriceQty / vwapQty);
					//if ( log.isDebugEnabled() )
					//	log.debug( "VWAP has [" + price + "," + qty + "] as old trade, latest VWAP " + lastVWAP );
				}
			}
		}, vwapTimeout );
	}
	...
	private void scheduleOrderSubmit() {
		timer.schedule( new TimerTask() {
			@Override public void run() {
				...
				double ordersToSend = simulatorOrdersPerFreq - lastSimulatedOrdersError;
	...
	final private ArrayDeque<TimerTask> ordersToBeCancelled = new ArrayDeque<TimerTask>( CANCEL_QUEUE );
	private void sendSimulatedOrder() {
		...
		// Calculate quantity
		final long qty = (Math.abs(rand.nextLong()) % (maxQty - minQty)) + minQty;
		
		
		final int  SHIFT = 3;
		final int  RANGE = 10;
		// Calculate price
		final long price = getVWAP() + (rand.nextInt(RANGE+1) - (RANGE/2)) + ((side) ? -SHIFT : SHIFT );

		//log.debug( "Price delta = " + (price - getVWAP()) );
		//System.out.println( "Order of qty " + qty + " from max " + maxQty + " min " + minQty );
		...
		o.setMatchableOrderListener( new MatchableOrderListener() {
			@Override public void trade(Order order, Order matchOrder, boolean isAggress, long price, long quantity) {
				addToVWAP( price, quantity );
			}
			@Override public void finish(Order order) {
				cancelTask.cancel();
			}
		});
